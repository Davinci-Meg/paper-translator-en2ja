...
- **2026-03-17** — 図の抽出をDocLayout-YOLOベースの検出方式に切り替えました。従来のpymupdfベースのアプローチではラスター画像しか抽出できませんでしたが、新しいYOLO + pdf2imageパイプラインにより、ラスター図とベクター図の両方を自動検出できるようになりました。さらにClaude Codeが論文のコンテキストを活用し、Figure番号の照合、検出結果の検証、漏れの補完を行うことで、自動検出と意味的な理解を組み合わせた処理を実現しています。
